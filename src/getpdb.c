...
int read_pdb(FILE *pdbInFile, Str *str, int coarse, int hydrogens)
{
    ...
    while(fgets(line, 80, pdbInFile) != 0) {
		...
		/* stop at END tag */
		if((strcmp(line, "TER   ") == 0)) {
			++ terflag; /* flag up TER occurrence */
			continue;
		}
		...
		/* program-internal chain identifier */
		str->atom[str->nAtom].systemChainID = (char)('A' + str->nChain);
        ...
        if (str->nAtom == 0) {
			if (! coarse) {
				fprintf(stdout, "\tPDB -> system chain mapping: '%c' -> '%c'\n",
					str->atom[str->nAtom].chainIdentifier[0], (str->nChain + 'A')); 
			}
		} else if ((str->atom[str->nAtom].chainIdentifier[0] != str->atom[str->nAtom - 1].chainIdentifier[0]) ||
			(terflag)) {
			++ str->nChain;
			if (! coarse)
				fprintf(stdout, "\tPDB -> system chain mapping: '%c' -> '%c'\n",
...
void read_structure(Arg *arg, Argpdb *argpdb, Str *pdb)
{
    ...
    if (! arg->silent) {
		if (argpdb->coarse) {
			fprintf(stdout, "\n\tcoarse-grained: PDB file: %s\n"
										"\tPDB file content:\n"
										"\t\tnumber of Calpha atoms = %d\n"
										"\t\tnumber of residues = %d\n"
										"\t\tnumber of chains = %d\n",
							arg->pdbInFileName, pdb->nAtom,
							pdb->nResidue, pdb->nChain);
		} else {
			fprintf(stdout, "\n\tfine-grained: PDB file: %s\n"
										"\tPDB file content:\n"
										"\t\tnumber of non-Hydrogen atoms = %d\n"
										"\t\tnumber of all atoms = %d\n"
										"\t\tnumber of residues = %d\n"
										"\t\tnumber of chains = %d\n",
